<template>
    <dl class="loan">
        ...
        <!--借款时长-->
        <dd>{{$t('m.investingdetail.JKSC')}}: {{items.loan_period}}{{$t('m.month')}}</dd>
        <!--借款利率-->
        ...
        <!--增加抵押-->
        <el-button v-if="(orderState !== 12) && isAddCollate" @click="addDYW(items)" type="primary" style="margin-left: 100px">{{$t('m.borrow.addCollateral')}}</el-button>
        </dd>
          <!--选择抵押物-->
...
.loan dd{display: block; line-height: 30px;}
</style>
