  ...
  <div slot="content">
    <div v-loading="loading" :element-loading-text="$t('m.loading')" style="min-height: 10vw;">
      <div v-if="!loading">
        ...
        <div class="earnings">
          <div v-if="investState === 2 || investState === 1">
            <!--收益-->
          ...
          <div v-if="investState === 2" class="margin-l20">
            <!--实际收益率-->
            {{$t('m.investList.realIncomeRate')}}
            : {{(investData.repay_interest_sum / (investData.amount_to_invest.amount / Math.pow(10, investData.asset_to_loan.precision)) * 1000).toFixed(1)}}‰
          </div>
        </div>
        <div class="earnings" v-if="investState === 1">
          <div v-if="investState === 1">
            <!--预计总收益-->
        ...
        <div class="pr">
          ...
          <img v-else-if="(investData.order_state===17 || investData.order_state===18) && investState === 2" src="/static/images/state_flow.png" class="recycle">
          <img v-else-if="(investData.order_state===7 || investData.order_state===8 || investData.order_state===9) && investState === 2" src="/static/images/state_complete.png" class="recycle">
          <img v-else-if="investData.order_state===19 && investState === 2" src="/static/images/state_del.png" class="recycle">
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      mortgeState () {
        if (Number(this.investState) === 2) {
          if (this.investData.order_state === 7 ||
            this.investData.order_state === 8 ||
            this.investData.order_state === 9) {
            return this.$t('m.borrowsuccess.thawed')
          } else if (this.investData.order_state === 13) {
            return this.$t('m.borrowsuccess.confiscated')
          } else if (this.investData.order_state === 17 || this.investData.order_state === 18 || this.items.order_state === 19) {
    ...
    methods: {
      ...
      borrowOrderId () {
        ...
        let isTrue = this.investData.order_state
