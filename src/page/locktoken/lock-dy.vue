    ...
    <div v-if="loading <= 0 || refershflag">
      ...
      <div class="layout">
        <el-card class="box-card box-card-left">
          <div class="dy_head">
            <div class="dy_head_item">
              ...
              <el-input
                ...
                size="medium"
                :placeholder="`${$t('m.lockasseted.minamount')}:${this.lockoptions.locktoken_min}`"
            ...
            <div class="dy_head_item">
              ...
              >
                <el-form-item
                :label="`${$t('m.orderList.HQ')}`">
                  <el-select size="medium" v-model="selectSymbol" value-key="id" :placeholder="$t('m.transfer.XZBZ')" @change="selectAsset()">
                    <el-option
          ...
          <div class="dy_main">
            <el-row>
              <el-col :span="4" class="title_info">{{$t('m.lockasseted.ratestr')}}:</el-col>
              <el-col :span="18" class="num_info" style="color: #E75441;">
                <span class="symbol_unit">{{selectSymbol.interestsymbol}}</span>
                {{this.rateString | formatLegalCurrency('', selectSymbol.interestprecision)}}
              </el-col>
            </el-row>
            <el-row>
          ...
          <div class="dy_btn"  @click="createitem(1)">{{$t('m.params.create_dy')}}</div>
        </el-card >
        <div class="dy_right">
          <el-card class="box-card" v-if="locksum.locktoken && locksum.locktoken[0]">
            <div class="dy_main">
              <el-row>
                <el-col :span="6" class="title_info">{{$t('m.lockasseted.total')}}:</el-col>
                <el-col :span="16" class="num_info">
                  <span class="symbol_unit">{{locksum.locktoken[0].total.symbol}}</span>
                  {{locksum.locktoken[0].total.amount | formatLegalCurrency3('', locksum.locktoken[0].total.precision)}}
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="6" class="title_info">{{$t('m.lockasseted.allinterest')}}:</el-col>
                <el-col :span="16" class="num_info" v-if="locksum.locktoken[0].interest.length < 1">0</el-col>
                <el-col :span="16" class="num_info" v-else>
                  <span v-for="(item,index) in locksum.locktoken[0].interest"  :key="index">
              ...
              <el-row>
                ...
                <el-col :span="16" class="num_info">{{locksum.locktoken[0].count}}</el-col>
              </el-row>
            </div>
          </el-card>
          <el-card class="box-card" style="margin-top:24px;">
            <div class="dy_main">
               <el-row>
                <el-col :span="6" class="title_info">{{$t('m.lockasseted.yearinterest')}}:</el-col>
                <el-col :span="16" class="num_info">{{rateInfo}}</el-col>
              </el-row>
              <el-row>
                <el-col :span="6" class="title_info">{{$t('m.lockasseted.maxlock')}}:</el-col>
                <el-col :span="16" class="num_info">
                  <span class="symbol_unit">{{selectSymbol.symbol}}</span>
                  {{lockoptions.locktoken_max | formatLegalCurrency('', selectSymbol.precision)}}
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="6" class="title_info">{{$t('m.lockasseted.minlock')}}:</el-col>
                <el-col :span="16" class="num_info">
                  <span class="symbol_unit">{{selectSymbol.symbol}}</span>
                  {{lockoptions.locktoken_min | formatLegalCurrency('', selectSymbol.precision)}}
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="6" class="title_info">
                  {{$t('m.lockasseted.interestcoin')}}
        ...
        <el-tabs v-model="activeTabName" @tab-click="handleClick" class="margin-t10 margin-lr10">
         <el-tab-pane :label="$t('m.orderList.HQJL')" name="first">
            <el-table
            :data="refunding1"
            class="curp"
            style="width: 100%;"
            ...
            <el-table-column width="180" align="center"  :label="$t('m.lockasseted.creattime')">
              <template slot-scope="scope">
                <span>{{scope.row.create_time}}</span>
              </template>
            </el-table-column>
            <el-table-column align="center"  :label="$t('m.lockasseted.coinDay')">
              <template slot-scope="scope">
               {{ scope.row.dy_interest.amount  | formatLegalCurrency3('', scope.row.interestprecision)}}
               <span class="symbol_unit">{{scope.row.interestsymbol}}</span>
              </template>
            </el-table-column>
            <el-table-column width="80" align="center"  :label="$t('m.lockasseted.interestrate')">
              <template slot-scope="scope">
          ...
          <el-table-column width="300" align="center"  :label="$t('m.orderList.operation')">
              <template slot-scope="scope" class="flex-h">
                <el-button type="text" @click="operator(0, scope.row)">{{$t('m.lockasseted.detail')}}</el-button>
                <el-button type="text" @click="operator(1, scope.row)">{{$t('m.lockasseted.change')}}</el-button>
                <el-button type="text" @click="operator(2, scope.row)">{{$t('m.lockasseted.removelock')}}</el-button>
                <el-button type="text" @click="operator(3, scope.row)">{{$t('m.lockasseted.addlock')}}</el-button>
                <el-button type="text" @click="operator(4, scope.row)">{{$t('m.lockasseted.dellock')}}</el-button>
              </template>
            </el-table-column>
            <div slot="empty">
              <p>{{$t('m.noData')}}</p>
         ...
         <el-tab-pane :label="$t('m.orderList.LSJL')" name="second">
            ...
            <el-table-column align="center"  :label="$t('m.lockasseted.removetime')">
              <template slot-scope="scope">
          ...
          <el-table-column width="100" align="center"  :label="$t('m.orderList.operation')">
              <template slot-scope="scope" class="flex-h">
                <el-button type="text" @click="operator(0, scope.row)">{{$t('m.lockasseted.detail')}}</el-button>
              </template>
            </el-table-column>
      ...
      <el-dialog
        ...
        width="30%"
      >
       <div class="layout_model" style="min-height: 10vw;">
        <div class="dy_main" style="margin-bottom:77px;">
          <el-row>
            <el-col :span="6" class="title_info">{{$t('m.lockasseted.lockamount')}}:</el-col>
            <el-col :span="16" class="num_info">
              <span class="symbol_unit">{{selectSymbol.symbol}}</span>
              {{ruleForm.lockamount | formatLegalCurrency('', selectSymbol.precision)}}
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="6" class="title_info">{{$t('m.drawDialog.available')}}:</el-col>
            <el-col :span="16" class="num_info">
               <span class="symbol_unit">{{selectSymbol.symbol}}</span>
              {{available | formatLegalCurrency('', selectSymbol.precision)}}
            </el-col>
         ...
         <el-row>
            <el-col :span="6" class="title_info">{{$t('m.borrow.fee')}}:</el-col>
            <el-col :span="16" class="num_info">
              <span class="symbol_unit">ZOS</span>
              {{this.feeZos | formatLegalCurrency('', 5)}}
            </el-col>
          </el-row>
         <el-row v-if='feed!==1.0'> <el-col :span="6" class="title_info">{{$t('m.lockasseted.feed')}}:</el-col>
           <el-col :span="16" class="num_info">
             {{this.feed}}
             <span class="symbol_unit">{{selectSymbol.symbol}}</span>/<span class="symbol_unit">{{selectSymbol.interestsymbol}}</span>
           </el-col>
         </el-row>
         <el-row> <el-col :span="6" class="title_info">{{$t('m.lockasseted.rate')}}:</el-col>
        ...
        <div class="confirm_box">
          <y-button :text="$t('m.cancel')" class="cancel_btn"  @btnClick="confirmDialog = false"></y-button>
          <y-button :text="$t('m.sure')"  class="determine_btn"   @btnClick="borrowSubmit" ></y-button>
        </div>
      </div>
     </el-dialog>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      depoValid () {
        if (Number(this.ruleForm.lockamount) >= 0) {
          ...
          if (this.ruleForm.lockamount.indexOf('.') !== -1) { // 如果存在小数点
            ...
            if (typeof (this.Symbol.precision) === 'undefined') {
              this.Symbol.precision = 2
            }
            if (newstr.length >= this.Symbol.precision + 1) {
              newstr = newstr.substring(0, this.Symbol.precision)
            }
      ...
      priceGet () {
        this.feed = 1.0
        if (this.selectSymbol.interestsymbol === this.selectSymbol.symbol || this.selectSymbol.symbol === 'ZOS') {
          this.getRate()
        } else {
          ZOSInstance.getAssetCoreFeed(this.selectSymbol.asset_id, this.selectSymbol.asset_precision, this.selectSymbol.lockoptions.pay_asset, this.selectSymbol.interest_precision)
            .then((res) => {
              this.feed = res
              this.getRate()
            })
      ...
      getRateByAmount (amount) {
        if (this.lockoptions && this.lockoptions.locktoken_rates.length > 0) {
          let value = Number(amount) * Math.pow(10, this.selectSymbol.asset_precision)
          this.rateSelect = ''
          this.ruleForm.rate = 0
          this.rate = 0
          this.lockoptions.locktoken_rates[0][1].forEach(item => {
          ...
          if (this.rateSelect === '') {
            let item = this.lockoptions.locktoken_rates[0]
            this.rateSelect += (item[1][0][1] / 10).toFixed(2) + '‰' + '/' + this.$t('m.year')
