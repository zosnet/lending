  ...
  <div slot="content" class="container-params" v-loading="loading > 0 && initDone === false" :element-loading-text="$t('m.loading')" style="min-height: 10vw;">
      <div>
       ...
       <el-table
          ...
          >
            <template slot-scope="scope" class="feel">
              <div >
                ...
                ></el-input> -->
                <el-button type="primary" size="medium" v-if="scope.row.key === 'locktoken_rates'" @click="dialogVisible = true">{{$t('m.address.edit')}}{{$t('m.lockasseted.yearinterest')}}</el-button>
        ...
        <!-- 编辑年化利率 -->
        <el-dialog
          :title="$t('m.address.edit') + $t('m.lockasseted.yearinterest')"
          :visible.sync="dialogVisible"
          width="80%">
          <rate ref="rate" :hoptions="options" @func="getMsgFormSon(arguments)"></rate>
        </el-dialog>
    </div>
      <!--输入资金密码-->
...
<script>
  ...
  import rate from '/path-page/Admin/locktoken/rate.vue'
  export default {
    components: {YButton, passwordDialog, rate},
    data () {
      return {
        loading: 1,
        initDone: false,
    ...
    methods: {
      ...
      getMsgFormSon (data) {
        ...
        this.msgFormSon = data[0]
        this.dialogVisible = data[1]
      },
      // 首页询问是否可以离开编辑页面
      canLeave () {
        return !this.isEdit
      ...
      getConfig () {
        ...
        Apis.instance().db_api().exec('get_locktoken_option', [this.selectedSymbol.id, this.lockmode, false]).then(res => {
          if (res) {
            ...
            this.options = {
              lock_mode: 1,
              locktoken_rates: [[[0, [[1, 160]]]], [[0, [[2, 180]]]], [[0, [[3, 250]]]], [[0, [[6, 300]]]], [[0, [[12, 500]]]]],
              max_rates: 100000000000000,
      ...
      // 格式化年化利率数据
      func (arr) {
        this.locktokenRatesNew = []
        for (var i = 0; i < arr.length; i++) {
          for (var j = 0; j < arr[i][1].length; j++) {
            this.locktokenRatesNew.push(arr[i][0] + '月' + (arr[i][1][j][1] / 1000).toFixed(2) + '‰')
          }
        }
        return this.locktokenRatesNew.toString()
      },
      /* 数据格式化 */
      tableDataForm (res) {
        /* 活期参数 */
        if (res.locktoken_rates === 0) {
          res.locktoken_rates = [[[0, [[1, 160]]]], [[0, [[2, 180]]]], [[0, [[3, 250]]]], [[0, [[6, 300]]]], [[0, [[12, 500]]]]]
        }
        ...
        this.func(res.locktoken_rates)
        this.lockoptions_dynamic = [
          // 年化率
          {
            name: this.$t('m.lockasseted.yearinterest'),
            key: 'locktoken_rates',
            value: this.func(res.locktoken_rates),
      ...
      tabaleRuleForm () {
        ...
        // 活期年化率
        // this.options['locktoken_rates'] = (this.options['locktoken_rates'][0][1][0][1] == this.lockoptions_dynamic[0].newValue || this.lockoptions_dynamic[0].newValue == '') ? this.options['locktoken_rates'] : [[0,[[1,this.lockoptions_dynamic[0].newValue*100]]]] //eslint-disable-line
        this.options['locktoken_rates'] = this.msgFormSon == '' ? this.options['locktoken_rates'] : this.msgFormSon //eslint-disable-line
        // 最大利息
